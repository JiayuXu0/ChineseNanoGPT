...
class WikiDataProcessor:
    ...
    def split_train_val(
        ...
        """分割训练集和验证集"""
        text = text[int(0.9999 * len(text)) :]  # noqa
        n = len(text)
        return text[: int(n * train_ratio)], text[int(n * train_ratio) :]
    ...
    def tokenize_and_save(
        ...
        # 转换并保存
        train_ids = train_inputs["input_ids"].numpy().astype(np.uint16)
        val_ids = val_inputs["input_ids"].numpy().astype(np.uint16)
